...
DGUI_USE_NAMESPACE
RoundedWidget::RoundedWidget(QWidget *parent, int r, int t, QPoint o) : QWidget(parent), radius(r), margin(0), type(t), offset(o), painter(nullptr)
{
...
void RoundedWidget::paintEvent(QPaintEvent *event)
{
    ...
    if(type)
    {
        auto tmpRect=rect;
        switch (type) {
        case TopTwo: tmpRect.adjust(0,rect.height()-radius,0,0); break;
        case BottomTwo: tmpRect.adjust(0,0,0,-(rect.height()-radius)); break;
        case LeftTwo: tmpRect.adjust(0,0,-(rect.width()-radius),0); break;
        case RightTwo: tmpRect.adjust(rect.width()-radius,0,0,0); break;
        }
