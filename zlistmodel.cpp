...
QVariant ZListModel::dataRole(ZNote &rt, int role) const
{
    if(role==Qt::UserRole)
    {
...
bool ZListModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role==Overview || role==Html || role==DisplayType)
    {
        ...
        items.modifyKth(index.row()+1,[str,role](ZNote *key){
            ...
            else if(role==DisplayType) key->toggleAttach();
...
{
    return index(items.queryIndex<InnerIndex>(idx)-1);  //Kth in items refers to (K-1)th in model
}
